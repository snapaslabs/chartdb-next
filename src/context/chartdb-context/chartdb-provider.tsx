...
> = ({ children, diagram, readonly: readonlyProp }) => {
    ...
    const applyRemoteUpdate: ChartDBContext['applyRemoteUpdate'] = useCallback(
        (incoming) => {
            if (new Date(incoming.updatedAt) <= new Date(diagramUpdatedAt)) return;

            if (!equal(incoming.tables ?? [], tables)) setTables(incoming.tables ?? []);
            if (!equal(incoming.relationships ?? [], relationships))
                setRelationships(incoming.relationships ?? []);
            if (!equal(incoming.areas ?? [], areas)) setAreas(incoming.areas ?? []);
            if (!equal(incoming.notes ?? [], notes)) setNotes(incoming.notes ?? []);
            if (!equal(incoming.customTypes ?? [], customTypes))
                setCustomTypes(incoming.customTypes ?? []);
            if (!equal(incoming.dependencies ?? [], dependencies))
                setDependencies(incoming.dependencies ?? []);
            if (incoming.name !== diagramName) setDiagramName(incoming.name);
            if (incoming.databaseType !== databaseType) setDatabaseType(incoming.databaseType);
            setDiagramUpdatedAt(new Date(incoming.updatedAt));
        },
        [
            tables, relationships, areas, notes, customTypes, dependencies,
            diagramName, databaseType, diagramUpdatedAt,
            setTables, setRelationships, setAreas, setNotes, setCustomTypes,
            setDependencies, setDiagramName, setDatabaseType, setDiagramUpdatedAt,
        ]
    );
