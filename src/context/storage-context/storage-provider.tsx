...
export const StorageProvider: React.FC<React.PropsWithChildren> = ({
    ...
    const db = useMemo(() => {
        ...
        // Add compound [id+diagramId] indexes for entity lookup queries
        dexieDB.version(14).stores({
            db_tables:
                '++id, diagramId, [id+diagramId], name, schema, x, y, fields, indexes, color, createdAt, width, comment, isView, isMaterializedView, order',
            db_relationships:
                '++id, diagramId, [id+diagramId], name, sourceSchema, sourceTableId, targetSchema, targetTableId, sourceFieldId, targetFieldId, type, createdAt',
            db_dependencies:
                '++id, diagramId, [id+diagramId], schema, tableId, dependentSchema, dependentTableId, createdAt',
            areas: '++id, diagramId, [id+diagramId], name, x, y, width, height, color',
            db_custom_types:
                '++id, diagramId, [id+diagramId], schema, type, kind, values, fields',
            notes: '++id, diagramId, [id+diagramId], content, x, y, width, height, color',
        });
    ...
    const addDiagram: StorageContext['addDiagram'] = useCallback(
        async ({ diagram }) => {
            ...
            await db.transaction(
                ...
                async () => {
                    // put() upserts — safe for both new and existing diagrams
                    ...
                    // Wipe existing sub-records so the cloud state is the exact
                    // source of truth (handles deleted tables, etc.)
                    await Promise.all([
                        db.db_tables.where('diagramId').equals(diagram.id).delete(),
                        db.db_relationships.where('diagramId').equals(diagram.id).delete(),
                        db.db_dependencies.where('diagramId').equals(diagram.id).delete(),
                        db.areas.where('diagramId').equals(diagram.id).delete(),
                        db.db_custom_types.where('diagramId').equals(diagram.id).delete(),
                    ...
                    await Promise.all([
                        ...
                        ...areas.map((area) =>
                            addArea({ diagramId: diagram.id, area })
                        ),
                        ...customTypes.map((customType) =>
                            addCustomType({ diagramId: diagram.id, customType })
                        ),
                        ...notes.map((note) =>
                            addNote({ diagramId: diagram.id, note })
                        ),
                    ]);
                }
            );
