...
const validateConfiguration = (): { provider: AIProvider } => {
    const openaiKey = window?.env?.OPENAI_API_KEY ?? OPENAI_API_KEY;
    const anthropicKey = window?.env?.ANTHROPIC_API_KEY ?? ANTHROPIC_API_KEY;
    const baseUrl = window?.env?.OPENAI_API_ENDPOINT ?? OPENAI_API_ENDPOINT;
    const modelName = window?.env?.LLM_MODEL_NAME ?? LLM_MODEL_NAME;

    if (baseUrl && modelName) return { provider: 'custom' };
    if (openaiKey) return { provider: 'openai' };
    if (anthropicKey) return { provider: 'anthropic' };

    throw new Error(
        'Configuration Error: Provide an OpenAI API key, an Anthropic API key, or both a custom endpoint and model name'
    );
};
...
export const exportSQL = async (
    ...
    const customModelName = window?.env?.LLM_MODEL_NAME ?? LLM_MODEL_NAME;
    ...
    if (provider === 'anthropic') {
        ...
        model = anthropic(customModelName || 'claude-haiku-4-5-20251001');
    } else if (provider === 'custom') {
        const { createOpenAI } = await import('@ai-sdk/openai');
        const openaiKey = window?.env?.OPENAI_API_KEY ?? OPENAI_API_KEY;
        const baseUrl = window?.env?.OPENAI_API_ENDPOINT ?? OPENAI_API_ENDPOINT;
        model = createOpenAI({ apiKey: openaiKey, baseURL: baseUrl })(customModelName);
    } else {
        const { createOpenAI } = await import('@ai-sdk/openai');
        const openaiKey = window?.env?.OPENAI_API_KEY ?? OPENAI_API_KEY;
        model = createOpenAI({ apiKey: openaiKey })(
            customModelName || 'gpt-4o-mini-2024-07-18'
        );
    }
