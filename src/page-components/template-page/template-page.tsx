...
interface TemplatePageComponentProps {
    template: Template | undefined;
}

const TemplatePageComponent: React.FC<TemplatePageComponentProps> = ({
    template,
}) => {
    ...
    const cloneTemplate = useCallback(async () => {
        if (APP_URL) {
            ...
            router.push(`/templates/clone/${templateSlug}`);
        }
    }, [router, templateSlug]);
...
export const TemplatePage: React.FC<TemplatePageProps> = ({ template }) => (
    <LocalConfigProvider>
        <ThemeProvider>
            <ReactFlowProvider>
                <TemplatePageComponent template={template} />
            </ReactFlowProvider>
        </ThemeProvider>
    </LocalConfigProvider>
);
